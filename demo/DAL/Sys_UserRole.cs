...
namespace zs.DAL
{
	...
	public partial class Sys_UserRole
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "Sys_UserRole"); 
		}
		...
		/// 增加一条数据
		/// </summary>
		public int Add(zs.Model.Sys_UserRole model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(zs.Model.Sys_UserRole model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public zs.Model.Sys_UserRole GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,UserID,RoleID from Sys_UserRole ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
