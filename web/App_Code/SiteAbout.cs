...
using Common.DEncrypt;
	...
	public class SiteAbout
	{
		...
		{
			string val = System.Web.HttpContext.Current.Request.Form[queryParamName] ;
			if ((val == null) || (val == ""))
				return defaultValue ;
			return val ;
		}
        ...
        {
            return int.Parse(gStr(queryParamName, defaultValue.ToString()));
        }
        /// <summary>
		...
		{
			SetCookie(varName, varValue, DateTime.Parse("2999-1-1"));
		}
        /// <summary>
		...
		public static bool IsSaveCookie
		{
			get
			{
                string val = System.Configuration.ConfigurationSettings.AppSettings["IsCookie"];
                if (val == "1")
                    return true;
                else
                    return false;
